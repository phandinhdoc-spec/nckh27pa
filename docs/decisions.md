...
|D07|Yêu cầu quyền Android cho SOS (điện thoại/SMS/vị trí/bản đồ)|Quyền runtime, từ chối vĩnh viễn, vị trí gần đúng, fallback bản đồ|ĐÃ QUYẾT (kỹ thuật, có thể phản biện): `CALL_PHONE`, `SEND_SMS`, vị trí foreground là ba quyền ĐỘC LẬP; `READ_PHONE_STATE` chỉ còn cho đa SIM (thiếu nó không chặn SMS); KHÔNG xin `ACCESS_BACKGROUND_LOCATION`; không xin quyền lúc khởi động; từ chối vĩnh viễn chỉ mở App Settings bằng một thao tác mới; chỉ cấp approximate vẫn là trạng thái dùng được; mở bản đồ thử Google Maps → `geo:` → browser; SIM call vẫn thủ công, bước gọi tự động của SOS là adapter thoại backend (D03). Chi tiết và ma trận suy giảm: docs/permission-flow.md.|
