function contact(row) {return row?{id:row.id,userId:row.user_id,name:row.name,relationship:row.relationship,phone:row.phone,receiveSos:!!row.receive_sos,isPrimary:!!row.is_primary}:null;}
export function contactRepository(db) {
 return {
  list: user=>db.prepare('SELECT * FROM emergency_contacts WHERE user_id=? ORDER BY rowid').all(user).map(contact),
  ...
  save(user,id,b) {db.prepare(`INSERT INTO emergency_contacts(user_id,id,name,relationship,phone,receive_sos,is_primary) VALUES(?,?,?,?,?,?,?) ON CONFLICT(user_id,id) DO UPDATE SET name=excluded.name,relationship=excluded.relationship,phone=excluded.phone,receive_sos=excluded.receive_sos,is_primary=excluded.is_primary`).run(user,id,b.name,b.relationship,b.phone,+b.receiveSos,+b.isPrimary);},
