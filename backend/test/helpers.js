...
export async function setup(t, overrides = {}, initialTime = 1000000) {
  let time = initialTime;
  const app = createApp({ config: { DB_PATH: ':memory:', HOST: '127.0.0.1', PORT: 0, ...overrides }, now: () => time });
  t.after(() => app.close());
  await app.listen();
  ...
  return { ...app, request, advance: ms => { time += ms; } };
}
